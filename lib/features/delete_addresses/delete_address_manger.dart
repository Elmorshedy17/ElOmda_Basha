...
class DeleteNewAddressManager implements Manager {
  final BehaviorSubject<DeleteNewAddressModel> _adsSubject =
      BehaviorSubject<DeleteNewAddressModel>();

  Stream<DeleteNewAddressModel> postData(id) {
    Stream.fromFuture(DeleteAddressRepo.postDeleteNewAddressData(id))
        .listen((v) {
      _adsSubject.add(v);
    });
